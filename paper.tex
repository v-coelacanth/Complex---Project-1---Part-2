...
{\bf Introduction } We model population growth when birth rate, death rate, and carrying capacity are all fixed.  Define $k$ to be the carrying capacity and $n_t$ to be size of the population at time $t$. Then the size of the population at generation $t$ is modeled by 
$
n_{t+1} = (birthrate - deathrate) (k n_t - n_t^2) / k,$ \  $\ t=0, 1, 2, \ldots$ .

Define the population size relative to the carrying capacity as  $x_t = n_t / k$ and define $R = birthrate - deathrate$.  Then we  get the following equation for $x_t$: 
\begin{eqnarray}
...
Depending on the value of $R$, the logistic map will generate a population size that is stable, periodic, or chaotic over time.  The behavior of the logistic map can vary a great deal with small changes in $x_0$.  This phenomenon is known as {\it sensitive dependence on initial conditions}, where small changes in the input lead to large changes in the output. 
...
% \begin{figure}[H]
 %\begin{subfigure}[b]{0.4\columnwidth}
   % \centering
    %\includegraphics[width=\textwidth]{figure1ab}
     %\caption{ LEFT}
 %   \label{figure1ab}
%\end{subfigure}
%
%\begin{subfigure}[b]{0.4\columnwidth}
 %\includegraphics[width=\textwidth]{figure1cd}
   % \caption{ RIGHT)}
 %\label{figure1cd}
 %\end{subfigure}
 %\caption{A big ole figure}
% \end{figure} 
...
For this project, we used the  model of growth define in \cite{Walker}  that uses an logistic growth model on a lattice of organisms coupled with influence from the entire population of organisms.  For each time step $n$, the size of population $i$ at time step $n$ is defined to be
\begin{eqnarray}
...
\end{eqnarray}
 ...
 \begin{eqnarray}
 m_n = {1 \over N} \sum_j^N f_j(x_{j,n})
\end{eqnarray}

The influence of $m_i$ on the entire system is quantified by the parameter $\epsilon$, the coupling strength.  
 
To do our simulation,  we set the carrying capacity $K$ to be 100 for all populations $i$, the number of populations $N = 1000$ and use $10000$ time steps.   We set the initial population size $x_i, 0$ to be $1$.   We also select a random fitness $r_i$ for each population $i$.   
...
% Figure for part 2
 ...
 \centering
    ...
    \caption{ Return map for selected values of  the global coupling strength $\epsilon$:  $\epsilon =0$ (red), $\epsilon = 0.1$ (blue), $\epsilon = 0.25$ (orange), $\epsilon = 0.225$ (aqua), $\epsilon = 0.25$ (dark green), $\epsilon = 3 $ (stars) and $\epsilon = 4$ (black)}
 \label{prettypicture}
\end{figure} 

This return map was using equations LIST EQUATION NUMBERS  in Python using Matplotlib.  
...
Vanessa did this and Kellin did that.   
...
Joseph T. Lizier  https://arxiv.org/abs/1408.3270
FIX THIS
...
\bibitem{Flake} G. Flake, The computational beauty of nature. Cambridge, Mass.: The MIT Press, 2011, pp. 139-151.
