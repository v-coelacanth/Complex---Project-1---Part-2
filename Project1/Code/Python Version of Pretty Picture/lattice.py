...
""" This program was written by Vanessa Job for CS523, February 2018.   It
    ...
    Top-Down Causation'.  The programmer also has the option of saving
    a run for a randomly chosen organism, though we did not end up using
    this version of that feature.  """
...
def compute_big_M(n, x, N):
    """ Compute M_n = (1/N) sum{j=1 to N}(x[j,n]), a.k.a., the sum of the
        nth row of x.  """
    M = 0
    """ Should replace this with a list comphrehension. """
...
def compute_little_m(x, n, N, r, K):
    """ Return little m, the mean field for the model. """
...
if __name__ == "__main__":
    ...
    if generate_return_map:
        """ Note: Instead of stars, we will use chartreuse and add the stars
            later. """
        ...
        for i in range(0,len(epsilons)):
            ...
            """ Keep the user updated on what's going on so they don't get
                bored. """
        ...
        print("Total time for run: ", t1-t0)
